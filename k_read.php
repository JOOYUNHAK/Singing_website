...
<?php
  ...
  $sql = "select * from k_pop where list_nu =".$bno;
  $h_sql = "select hit from k_pop where list_nu =".$bno;

  $h_result = mysqli_query($con, $h_sql);

  while($h_row = mysqli_fetch_array($h_result)){
        $u_hit = $h_row['hit'] + 1;
        $update = "update k_pop set hit = '".$u_hit."' where list_nu = '".$bno."'";
        $u_result = mysqli_query($con, $update);
        $u_sql = "select hit from k_pop where list_nu =".$bno;
  ...
  ?>
  <div id = "board">
    <h1 align=center>***K-POP게시판***</h1><br><br>
  ...
  <div id="bar">
    <ul>
      <li><a href="k_pop.php?list_nu=<?php echo $row['list_nu']; ?>">[목록으로]</a></li>
      <li><a href="k_modify.php?list_nu=<?php echo $row['list_nu']; ?>">[수정하기]</a></li>
      <li><a href="k_delete.php?list_nu=<?php echo $row['list_nu']; ?>">[삭제하기]</a></li>
      <li><a href="k_up.php?list_nu=<?php echo $row['list_nu']; ?>">[추천하기]</a></li>
      <li><a href="k_error.php?list_nu=<?php echo $row['list_nu']; ?>">[신고하기]</a></li>
    </ul>
  </div>
</div>
<?php } ?>
