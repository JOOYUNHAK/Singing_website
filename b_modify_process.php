<?php
    ...
    $sql = "update ballad set title='$title', content='$content',
                          w_date='$date' where list_nu=$list_nu";
    $result = mysqli_query($con, $sql);
    //베스트 게시판 수정
    $b_sql = "select * from bestboard where list_nu = $list_nu and category = 'ballad'";
    ...
    if($b_result){ //만약 베스트 게시판에 있는 글이면 삭제
        ...
        $b_hit = $b_row['hit'];
        $b_error = $b_row['error'];
        $b_delete = "delete from bestboard where list_nu = $list_nu and category = 'ballad'";
        ...
        $b_insert = " insert into bestboard(list_nu, id, pwd, title, content, w_date, hit, up, error, category)
                        values('$list_nu', '$b_id', '$b_pwd', '$title', '$content', '$date', '$b_hit', '$b_up', '$b_error', 'ballad')";
...
?>
        <script>
            alert("수정되었습니다.");
            location.replace("b_read.php?list_nu=<?=$list_nu?>");</script>
<?php    }
    else {
        echo "수정에 실패하였습니다";
    }
