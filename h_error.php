<?php
    ...
    if(!isset($_SESSION['UserId']))
    {
      ...
      $error = "select * from hiphop where list_nu=".$bno;
      $result = mysqli_query($con, $error);

      while($row = mysqli_fetch_array($result)){

      $exist_member = $row['error_member']; //기존에 신고한 아이디 명단
      $mem_sql = "select error_member from hiphop where list_nu=$bno";
      ...
      if($find[0] != 0){
        echo "<script> alert('신고는 한번만 가능합니다.');
              location.href='hiphop.php'; </script>";
      }

      else{
      $update_member = $exist_member.$member; //추천이 처음이라면 기존 신고 명단에 추가
      $add = "update hiphop set error_member = '$update_member' where list_nu = $bno";
      $a_result = mysqli_query($con, $add);

      $u_error = $row['error'] + 1;
      $update = "update hiphop set error = '".$u_error."' where list_nu = '".$bno."'";
      $u_result = mysqli_query($con, $update);
      //베스트게시판
      $b_sql = "select * from bestboard where list_nu= $bno and category = 'hiphop'";
      $b_result = mysqli_query($con, $b_sql);
      if($b_result){
        $b_update = "update bestboard set error = $u_error where list_nu = $bno and category ='hiphop'";
        $b_result2 = mysqli_query($con, $b_update);}

      if($u_error == 5 ){
          $d_sql = "delete from hiphop where list_nu='$bno'"; //다수의 신고 삭제
          $d_result = mysqli_query($con, $d_sql);

          $f_sql = "select * from hiphop where list_nu = $bno and category = 'hiphop'"; //베스트게시판 같이 삭제
          $f_result = mysqli_query($con, $f_sql);
          if($f_result){
                $db_sql = "delete from bestboard where list_nu = $bno and category = 'hiphop'";
