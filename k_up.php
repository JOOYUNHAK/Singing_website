  ...
  if(!isset($_SESSION['UserId']))
    {
      ...
      $thumb = "select * from k_pop where list_nu=".$bno;
      $result = mysqli_query($con, $thumb);

      while($row = mysqli_fetch_array($result)){

      $exist_member = $row['up_member']; //기존에 추천한 아이디 명단
      $mem_sql = "select up_member from k_pop where list_nu=$bno";
      ...
      $add = "update k_pop set up_member = '$update_member' where list_nu = $bno";
      $a_result = mysqli_query($con, $add);

      $u_up = $row['up'] + 1;
      $update = "update k_pop set up = '".$u_up."' where list_nu = '".$bno."'";
      ...
      if($b_result){
        $b_update = "update bestboard set up = $u_up where list_nu = $bno and category ='k-pop'";
