<?php
    ...
    if(!isset($_SESSION['UserId']))
    {
      ...
      $error = "select * from k_pop where list_nu=".$bno;
      $result = mysqli_query($con, $error);

      while($row = mysqli_fetch_array($result)){

      $exist_member = $row['error_member']; //기존에 신고한 아이디 명단
      $mem_sql = "select error_member from k_pop where list_nu=$bno";
      ...
      if($find[0] != 0){
        echo "<script> alert('신고는 한번만 가능합니다.');
              location.href='k_pop.php'; </script>";
      }

      else{
      $update_member = $exist_member.$member; //신고가 처음이라면 기존 추천 명단에 추가
      $add = "update k_pop set error_member = '$update_member' where list_nu = $bno";
      $a_result = mysqli_query($con, $add);

      $u_error = $row['error'] + 1;
      $update = "update k_pop set error = '".$u_error."' where list_nu = '".$bno."'";
      $u_result = mysqli_query($con, $update);
      //베스트게시판
      $b_sql = "select * from bestboard where list_nu= $bno and category = 'k-pop'";
      $b_result = mysqli_query($con, $b_sql);
      if($b_result){
        $b_update = "update bestboard set error = $u_error where list_nu = $bno and category ='k-pop'";
        $b_result2 = mysqli_query($con, $b_update);}

      if($u_error == 5 ){
          $d_sql = "delete from k_pop where list_nu='$bno'"; //다수의 신고 삭제
          $d_result = mysqli_query($con, $d_sql);

          $f_sql = "select * from k_pop where list_nu = $bno and category = 'k-pop'"; //베스트게시판 같이 삭제
          $f_result = mysqli_query($con, $f_sql);
          if($f_result){
                $db_sql = "delete from bestboard where list_nu = $bno and category = 'k-pop'";
