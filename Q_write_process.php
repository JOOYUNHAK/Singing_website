...
$auto = "alter table QA auto_increment = 1"; //글 번호 초기화
...
if(strcmp($now_id, $id) != 0){ ?>
      ...
      while($p_row = mysqli_fetch_array($p_sql)){
            ...
            <?php }
                ...
                $sql = "insert into QA(title, pwd, id, content, w_date, category, state) values
                        ('$title', '$pwd', '$id', '$content', '$date', '$category', 'false')";
                $result = mysqli_query($con, $sql);
                    if($result){ ?>
                          <script> alert("질문이 등록되었습니다");
                          location.replace("QA.php"); </script>
            <?php
                   }
                }
          }
